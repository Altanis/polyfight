...
function CheckCustomCharacters(characters) {
    // Unique characters
...
function JSZ_OBFUSCATOR(inputCode, options) {
    if (inputCode === "") {
        return "";
    }
...
(async function() {
    let code = fs.readFileSync("./views/public/client.js", "utf8");

    if (process.env.PROD !== "true")
    {
        fs.writeFileSync("./views/public/client.js", code);
        return console.log("Not in production mode, skipping obfuscation.");
    };

    const closure_compiler = require("google-closure-compiler").compiler;
    const js_obfuscator = require("javascript-obfuscator").obfuscate;
    const js_confuser = require("js-confuser").obfuscate;
    const terser_compress = require("terser").minify;
    const babel_transform = require("@babel/core").transformSync;
    // const esprima_parse = require("esprima").parseScript;

    const js_confuser_settings = {
        target: "browser",
        ...
        // opaquePredicates: 0.45,
        // controlFlowFlattening: 0.06,
        // stringCompression: 0.23,
        stringConcealing: false,
        globalConcealing: true,
        // renameVariables: true,
        
        /* broken opts */
        // movedDeclarations: true, //broken with gcc
        // dispatcher: true,
        // flatten: true,
        // renameVariables: true,
        // identifierGenerator: "number",
        // shuffle: 0.5,
    ...
    const terser_settings = {
        mangle: {
            toplevel: true,
            properties:
            {
                regex: new RegExp("^(game_server_config|assets|images|home|polyfight_canvas|context|minimap_canvas|minimap_context|score_canvas|score_context|stats_canvas|stats_context|upgrade_tanks_canvas|upgrade_tanks_context|scoreboard_canvas|scoreboard_context|phase|notifications|last_render|deltas|screen_opacity|target_screen_opacity|inputs|auto_fire|auto_spin|mouse|x|y|stats|stats_value|max_stats_value|preloaded_stats|upgrade_colours|_r|_g|_b|available_stat_points|max_stat_points|display|x_offset|target_x_offset|upgrade_width|upgrade_tanks|current_upgrades|angle|direction|dismissed|chat_information|typing|force_open|scoreboard|leader_direction|leader_intersection|clans|clan_requests|stats_buttons|upgrade_tanks_buttons|animation_metadata|title|bound|change|value|zoom_pos|canvas_transform|server|global|client|entity|log_update|id|radius|target_radius|health|target_health|max_health|last_health_percent|position|target_position|velocity|target_velocity|target_angle|spawn_timestamp|fov|target_fov|health_bar_opacity|target_health_bar_opacity|updated|type|ticks|dying_phase|opacity|target_opacity|dont_revive|damage_blend|target_damage_blend|time_alive|identity_id|show_name|show_health|z_index|census_map|surroundings|name|spawning|killer|killer_name|score|target_score|turret_length|target_turret_length|latest_projectile_id|kills|level_width|old_entity_id|zoom_translation|radial_translation|target_radial_translation|message|polyfight_elements|container|disconnect_elements|connect_elements|disconnected_message|game_title|textbox|play_button|google_signin|clan_button|clan_modal|clan_modal_close|clan_textbox|clan_create|clan_list|clan_modal_title|settings_button|settings_close|settings_modal|themes_dropdown|theme_name|new_theme|delete_theme|theme_colours|import_theme|export_theme|nyc_button|amsterdam_button|changelog_button|info_button|game_buttons|modal|modal_header|modal_body|modal_x|last_key|key_held|purge_all_stats|polyfight_connection|packet_handler|initial_packet|manager|stream|buffer|index|cipher|lcg|seed|modulus|multiplier|increment|connected|retries|failed|last_ping|latencyRenderPhase|Home|Game|Dead|OutgoingPacketHeader|Spawn|Input|Stat|Upgrades|Chat|Ping|Clan|IncomingPacketHeader|Update|Stat|Upgrades|Notification|ServerInfo|Pong|CensusProperties|Position|Velocity|Angle|Radius|Health|MaxHealth|Alive|IdentityId|Name|Fov|Score|Spawning|Invisible|Turrets|Message|ShapeType|Shiny|Owner|Turret|ProjectileType|Clan|FieldType|Create|Update|Delete|EntityType|Generic|Tank|Shape|Projectile|ShapeType|Unknown|Square|Triangle|SmallCrasher|LargeCrasher|Pentagon|AlphaPentagon|ProjectileType|Bullet|AutoBullet|Drone|Trap|NecromancerDrone|Minion|ShinyType|Normal|Shiny|Mythical|Inputs|Shoot|Up|Down|Left|Right|Repel|LevelUp|DyingPhase|None|Alive|Dying|Dead|UpgradeStats|HealthRegen|MaxHealth|BodyDamage|ProjectileSpeed|ProjectilePenetration|ProjectileDamage|ProjectileReload|MovementSpeed|Fov|Sentinel|EntityIdentityIds|Projectile|Square|Triangle|SmallCrasher|LargeCrasher|Pentagon|AlphaPentagon|BasicTank|Twin|FlankGuard|Sniper|MachineGun|TripleShot|QuadTank|TwinFlank|Assassin|Hunter|Destroyer|Gunner|Triplet|PentaShot|SpreadShot|OctoTank|TripleTwin|Ranger|Stalker|Predator|Streamliner|Sprayer|Annihilator|TriAngle|Booster|Fighter|Smasher|Landmine|Spike|MegaSmasher|AutoGunner|Overseer|Overlord|Manager|Hybrid|Trapper|TriTrapper|GunnerTrapper|Overtrapper|MegaTrapper|AutoTrapper|Battleship|Necromancer|Factory|SpinType|None|Normal|Tbag|ConfigOptionTypes|Colour|Number|Boolean)$"),
            }
        },
        compress: {
            defaults: false,
            ...
            drop_console: true,
            drop_debugger: true,
            ecma: 5, //2015 or greater is possible
            evaluate: true,
            expression: false,
            global_defs: {},
            hoist_funs: true,
            ...
            module: false,
            negate_iife: true,
            passes: 3, /* higher = better compressed, but less performant at compile time */
            properties: true,
            pure_funcs: null, /* or array, i.e. ["Math.floor"] */
            pure_getters: "strict",
            reduce_vars: true,
            reduce_funcs: true,
            sequences: true,
            ...
            unused: true
        } /* https://www.npmjs.com/package/terser#parse-options */
    };

    // const js_obfuscator_settings = {
    //     compact: true,
    ...
    //     stringArrayCallsTransformThreshold: 0.01,
    ...
    //     stringArrayWrappersType: "variable", //or "function"
        
    //     unicodeEscapeSequence: false,
    //     seed: Math.random() * 9999999 | 0
    // };
    // const js_obfuscator_settings = {
    //     compact: true,
    //     controlFlowFlattening: true,
    ...
    async function closure_obfuscate(code)
    {
        return new Promise((res, rej) => {
            const closureCompiler = new closure_compiler({
                js: "./views/public/client.js",
                compilation_level: 'ADVANCED'
            });
            closureCompiler.run((exitCode, stdOut, stdErr) => {
                if (exitCode === 0) res(stdOut);
                else rej(stdErr);
            });
        });
    }

    function js_obfuscator_exec(code)
    {
        return js_obfuscator(code, js_obfuscator_settings).getObfuscatedCode();
    }

    async function js_confuser_exec(code)
    {
        return await js_confuser(code, js_confuser_settings);
    }

    async function terser_exec(bundle)
    {
        return new Promise((res, rej) => {
            terser_compress(bundle, terser_settings)
            .then(({code}) => res(code))
            .catch(e => {
                console.error("COULDNT TERSER:", e);
                res(bundle);
            });
        });
    }

    function babel_exec(code)
    {
        return babel_transform(code, babel_settings).code;
    }

    function jfogs_exec(code)
    {
        return jfogs_obfuscate(code, { type: "reverse" });
    }
    ...
    async function obfuscate_bundle(bundle)
    {
        for (let i = 0; i < steps.length; ++i) {
            const step = steps[i];
            console.log(`Executing "${step.name}" (${i + 1}/${steps.length})...`);
            if (step.constructor.name == "AsyncFunction") {
                bundle = await step(bundle);
            } else {
                bundle = step(bundle);
            }
        }

        bundle += "\n//# sourceURL=https://www.gstatic.com/recaptcha/releases/4rwLQsl5N_ccppoTAwwwMrEN/recaptcha__en.js";
        return bundle;
    };

    code = await obfuscate_bundle(code);
    fs.writeFileSync("./views/public/client.js", code);
